...
class Header(nn.Module):
    def __init__(self, in_features, f_features):
        super(Header, self).__init__()
        self.fc = nn.Linear(in_features, f_features)
        self.features = nn.BatchNorm1d(f_features, eps=1e-05)
...
class CosFace(nn.Module):
    def __init__(self, in_features, out_features, s=64.0, m=0.35): # todo check if
    ...
    def forward(self, embbedings, label):
        emb = l2_norm(embbedings, axis=1)
        kernel_norm = l2_norm(self.kernel, axis=0)
        cos_theta = torch.mm(emb, kernel_norm)
        cos_theta = cos_theta.clamp(-1, 1)  # for numerical stability
        index = torch.where(label != -1)[0]
        m_hot = torch.zeros(index.size()[0], cos_theta.size()[1], device=cos_theta.device)
...
class AdaFace(nn.Module):
    def __init__( self, embedding_size=512, classnum=70722, m=0.4, h=0.333, s=64.0, t_alpha=1.0,):
...
class BinaryCrossEntropyHeader(nn.Module):
    def __init__(self, in_features, out_features):
        super(BinaryCrossEntropyHeader, self).__init__()
    ...
    def forward(self, x, target):
        ...
        loss = self.loss_fn(logits, target_one_hot)
        return logits, loss
